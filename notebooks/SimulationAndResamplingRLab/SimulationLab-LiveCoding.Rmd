...
ssevals <- sapply(1:4,function(x) sum((x - testData)**2))
...
ggplot(myUniformSamplesDf,aes(samples)) +
  geom_histogram(bins=50)
```
...
ggplot(myResultsDf,aes(results)) +
  geom_histogram(bins=50) +
  xlim(-1,1)
```
...
We could also do this using the built-in `boot()` function from R. It's a bit tricky as it requires us to build a function do compute our statistic on the resampled data.
