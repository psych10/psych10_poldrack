...
RMSElinear = sqrt(mean((NHANES_sample$Height -lmResultLinear$fitted.values)**2))
RMSElinear
```
...
sampleFit = function(){
  NHANES_sample2 =NHANES %>%
    ...
    select(Age,Height,Gender) %>%
    sample_n(sampleSize)
  rmse=sqrt(mean((NHANES_sample2$Height -predict(lmResultLinear,newdata=NHANES_sample2))**2))
}

RMSElinearNew = replicate(100,sampleFit())
mean(RMSElinearNew)
```
...
RMSEloo = sqrt(mean((cvPredictions$Height-cvPredictions$predictedLOO)**2))
RMSEloo
```
