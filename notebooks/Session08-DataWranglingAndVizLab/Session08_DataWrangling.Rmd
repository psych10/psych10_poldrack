...
```{r}
attitudeData=read.csv('https://psych10.github.io/data/Session08_DataWrangling/statsAttitude.txt',stringsAsFactors = FALSE)
head(attitudeData)
dim(attitudeData)
```
...
If you look closely at the data, you can see that some of the participants have some missing responses.  We can count them up for each individual.
...
We can see from the table that there are two participants who are missing responses for all 29 questions.  Let's see which ones, using the which() function which tells us which elements the vector match a particular test.

```{r}
which(numNAvals==29)
attitudeData[which(numNAvals==29),]
```


There are fancy techniques for trying to guess the value of missing data (known as "imputation") but since the number of participants with missing values is small, let's just drop those participants from the list. 
...
This looks like responses are all over the place - but remember from above that some of the items require recoding.  The raw responses were on the 1-7 scale, which for those items we need to reverse by subtracting the raw score from 8 (such that 7 becomes 1 and 1 becomes 7).  To do this, we first need to create a column that contains the question numbers.For this we use the mutate() function, which creates new variables based on the existing variables.  Then we reverse the responses for those items that are in our list of reverse-scored items. We do this using the %in% operator, which tests whether a vector contains a specific value.
...
Now let's try the more complex example, where we check that the question numbers for reversed items are correct. The subset() function allows us to select only a subset of cases; in this example we are only selecting items where the raw response is different from the response, which means that its value has been reversed.  Then we make sure that its question number is part of the list of questions that should have been reversed.
