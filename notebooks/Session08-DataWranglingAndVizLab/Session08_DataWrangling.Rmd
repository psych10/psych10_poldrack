...
newNames = paste0('statt',1:nQuestions)
...
```{r}
which(numNAvals==nQuestions)
attitudeData[which(numNAvals==nQuestions),]
```
...
Now let's try the more complex example, where we check that the question numbers for reversed items are correct. The filter() function allows us to select only a subset of cases; in this example we are filtering for items where the raw response is different from the response, which means that its value has been reversed.  Then we make sure that its question number is part of the list of questions that should have been reversed.
