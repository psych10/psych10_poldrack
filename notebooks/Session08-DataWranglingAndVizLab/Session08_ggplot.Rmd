...
How do you choose which **geom** to use? ggplot allows you to choose from a number of geoms (short for geometries). This choice will determine what sort of plot you create. 
...
A note on defaults: The default statistic (or "stat") underlying geom_density is called "density" -- not surprising. The default stat for geom_historam is "count". What do you think would happen if you overrode the default and set stat="count"?
...
```{r}
ggplot(mpg, aes(manufacturer, displ)) +
  facet_grid(year~.) + # we split up our bar plot into a series of plots, one per class
  geom_bar(stat = "summary", fun.y = "mean")
```
...
# summarise data
mpg_summary <- mpg %>%
  group_by(manufacturer) %>% # group the data by car manufacturer
  summarise(n = n(), # get the count for each group
            mean_displ = mean(displ), # get the mean of each group (what we did earlier with stat="summary")
            sd_displ = sd(displ)) # get the standard deviation of each group

limits <- limits <- aes(ymin=mean_displ-1.96*sd_displ/sqrt(n), 
              ymax=mean_displ+1.96*sd_displ/sqrt(n)) # compute the upper and lower limits of the error bars; we'll talk about how this is done later in the course

ggplot(mpg_summary, aes(manufacturer, mean_displ)) +
  geom_bar(stat = "identity") + # now that we gave ggplot the mean for each group, we set stat = "identity"
